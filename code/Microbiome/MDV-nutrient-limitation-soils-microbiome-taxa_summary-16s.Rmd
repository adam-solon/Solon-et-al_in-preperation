...
df.cyano.fam.bn.1 <- bind_rows(df.cyano.fam.bn, row.1)

#remove data from plate that was overwatered
df.cyano.fam.bn.1 <- df.cyano.fam.bn.1 %>%
  mutate(
    Family = if_else(Sample == "BNM_11", NA_character_, Family),
    FamSum = if_else(Sample == "BNM_11", NA_real_, FamSum)) %>%
  mutate(Sample = if_else(Sample == "BNM_11", "BNM_22", Sample))

# Reorder 'treatment'
df.cyano.fam.bn.1$treatment <- factor(df.cyano.fam.bn.1$treatment,
                               levels = c("bulk", "CTRL", "N", "P", "NP"))

#reorder factor levels of Phylum
df.cyano.fam.bn.1$Family <- factor(df.cyano.fam.bn.1$Family, levels=c("Nostocaceae",
                                                                "Nodosilineaceae",
                                                                "Phormidesmiaceae",
                                                                "Phormidiaceae",
                                                                "Leptolyngbyaceae",
                                                                ...
                                                                "other"))
#create a color palette
pal.bn <- c('#054907', "#0072B2", '#F0E68C', "#B8860B", "#7B68EE", '#FFA500', '#01153E', 'grey50') 

#annotations for missing plates
df.anno <- data.frame(
  Sample = c("BNM_21", "BNM_22"),  # Replace with actual x-axis values
  FamSum = c(10, 10),                 # Replace with desired y-axis positions
  treatment = c("P", "NP"), # Facet categories to appear in
  label = "x"
)

#treatment factor
df.anno$treatment <- factor(df.anno$treatment, levels = levels(df.cyano.fam.bn.1$treatment))

# Reorder 'treatment'
df.cyano.fam.bn.1$treatment <- factor(df.cyano.fam.bn.1$treatment,
                               levels = c("bulk", "CTRL", "N", "P", "NP"))
# Create the bar plot
p.fam.bn <- ggplot(data = df.cyano.fam.bn.1, aes(x = Sample, y = FamSum, fill = Family)) +
   geom_text(data = df.anno, aes(x = Sample, y = FamSum, label = label), color = "black", size = 6, inherit.aes = FALSE) +  
  geom_bar(stat = "identity", position = "stack") +
  scale_fill_manual(values = pal.bn, na.translate = FALSE) +
  scale_y_continuous(expand = c(0,0)) +
...
#save R object
saveRDS(p.fam.bn, paste0(sub.directory.2.1.1.1.2.2.fp, "/p.fam.bn.rds"))
...
#create a color palette
pal.hr <- c('#054907', "#0072B2", '#F0E68C', "#B8860B", "#7B68EE", '#FFA500', 'grey50') 
...
#save R object
saveRDS(p.fam.hr, paste0(sub.directory.2.1.1.1.2.2.fp, "/p.fam.hr.rds"))
...
#create a color palette
pal.fx <- c('#054907', "#0072B2", '#F0E68C', "#B8860B", "#7B68EE", '#FFA500', 'grey50') 
...
#save R object
saveRDS(p.fam.fx, paste0(sub.directory.2.1.1.1.2.2.fp, "/p.fam.fx.rds"))
...
#save R object
saveRDS(combined_plot, paste0(sub.directory.2.1.1.1.2.2.fp, "/p.16s.rds"))

```

```{r combine plots}
# Modify each plot to remove extra margins and spacing
p.fam.bn.2 <- p.fam.bn.1 + theme(legend.position = "none",
...
p.fam.comb <- p.fam.bn.2 / p.fam.hr.1 / p.fam.fx.1 
...
file_names <- c("/MDV-nutrient-limitation-soils-taxa-summary-16s-facet", 
                "/MDV-nutrient-limitation-soils-taxa-summary-16s-no-legend")
...
df.cyano.fam.2 <- df.cyano.fam.1[df.cyano.fam.1$Sample != "BNM_11", ]

#calculate mean and se
cyano_fam_sum_stats <- df.cyano.fam.2 %>%
...
write.csv(cyano_fam_sum_stats,  file.path(sub.directory.2.1.1.1.2.1.fp, "Cyanobacteria_Family_summary_stats.csv"), row.names = FALSE)
...
for (loc in unique(df.cyano.fam.2$location)) {
  for (fam in unique(df.cyano.fam.2$Family)) {
    ...
    # Subset data
    subset_data <- subset(df.cyano.fam.2, location == loc & Family == fam)
