...
subtitle: "  \n Processing of 16S DNA-seq Data"
author: "Adam J. Solon"
date: "`r Sys.Date()`"
...
# Script Summary  
This script processes the 16S SSU rRNA gene Amplicon Sequence Variants (ASVs) table output from DADA2, from DNA sequenced from sediments collected in Taylor Valley of the McMurdo Dry Valleys, Antarctica.
...
F:/Projects/Apple_Tree/soil_microbial_communities/data/Seq-run-Mar-2025-16s.txt
```{r input data files}
# input data files
# ASV Table (from DADA2)
asvTable.fp <- paste0(sub.directory.1.1.1.fp, "/MDV-soil-nutrient-limitation-ASVtab-wTax-16s.txt") 
...
t.1 <- t %>%
  separate(taxonomy, into = c("Domain", "Phylum", "Class", "Order", "Family", "Genus", "ASV_ID"), sep = ";", remove = TRUE)
...
a.7 <- subset(a.6, !(SampleID %in% m.blanks$SampleID )) 
...
a.blanks.T.1 <- a.blanks.T %>%
  dplyr::select(ASV_ID, K710:Genus)

# Define column names
columns_to_check <- c("K710", "B905", "B906.1", "B906.2", "B910.1", "B911.1", "B919")
...
a.8 <- subset(a.8, !(ASV_ID %in% c$ASV_ID)) 
...
m.3 <- m.2[m.2$AltID %in% colnames(a.13), ]
...
if (length(non_matching) == 0) {
  cat("All SampleID values match the column names in dataframe 'a.13'.\n")
} else {
  cat("The following SampleID values do not match any column names in dataframe 'a.13':\n")
  print(non_matching)
}
...
# save as an R file
saveRDS(a.13, paste0(sub.directory.2.1.1.1.2.fp, "/ASV.table.13.wTax.filtered.rds"))

#save as .txt file
write.table(a.13, file = paste0(sub.directory.2.1.1.1.2.fp, "/MDV-nurtient-limitation-soils-ASv-tab-16s.txt"), 
            sep = "\t", row.names = FALSE, col.names = TRUE)
