...
datasets.e.1 <- list(df.cn.3 = c("umol.C_g.soil", "umol.N_g.soil", "C_to_N"),
...
datasets.e.2 <- list(df.cn.3, m.3)
locations <- c("Fryxell", "Bonney")

# Define variables for each test
welch_vars <- list("Fryxell" = "C_to_N", "Bonney" = c("umol.N_g.soil", "umol.C_g.soil"))
...
```{r rob reg model}
# Define datasets and treatment levels
datasets.e.3 <- list(df.cn.3, m.3)
locations <- c("Fryxell", "Bonney")
robust_vars <- list("Fryxell" = "umol.C_g.soil", "Bonney" = "ngDNA_per_g_soil")
...
custom_treatment_colors_list <- list(
  Bonney  = c("bulk" = "grey50", "CTRL" = "tan", "N" = "tan", "P" = "#01665E", "NP" = "#01665E"),
  Hoare   = c("bulk" = "grey50", "CTRL" = "tan", "N" = "#01665E", "P" = "tan", "NP" = "#01665E"),
  Fryxell = c("bulk" = "grey50", "CTRL" = "tan", "N" = "#01665E", "P" = "#01665E", "NP" = "#01665E")
)
...
df_cn_filtered <- df.cn.3 %>% filter(Study == "Solon" & !is.na(treatment))
...
```{r define objects p, dna}
# Define selected variables for df.cn.2
selected_vars_dna <- c("ngDNA_per_g_soil")

# Define fixed y-axis ranges
y_axis_limits_d <- list(
  "ngDNA_per_g_soil" = c(0, 5000)
)

y_breaks_d <- list(
  "ngDNA_per_g_soil" = c(0, 1000, 2000, 3000, 4000, 5000)
)

```

```{r plot dna}
# Filter dataset for the study
df_m_filtered <- m.3 %>%
  filter(Study == "Solon" & !is.na(treatment)) %>%
  mutate(treatment = factor(treatment, levels = c("bulk", "CTRL", "N", "P", "NP")))

# Initialize list to store plots
treatment_plots_dna <- list()

for (var in selected_vars_dna) {
  for (location_name in c("Bonney", "Hoare", "Fryxell")) {
    df_location_filtered <- df_m_filtered %>% filter(Location == location_name)

    if (!(var %in% colnames(df_location_filtered))) {
      print(paste("Skipping variable", var, "in location", location_name, "since it is not found in the dataframe"))
      next
    }
    ...
    if (nrow(df_var_filtered) == 0) {
      print(paste("Skipping variable", var, "in location", location_name, "since no valid data remains after filtering"))
      next
    }
    ...
    p <- ggplot(df_var_filtered, aes(x = treatment, y = .data[[var]], fill = treatment)) +
      ...
      geom_pointrange(data = summary_stats, aes(x = treatment, y = mean, ymin = mean - se, ymax = mean + se, fill = treatment),
                      size = 2, shape = 21, color = "black", stroke = 0.5, alpha = 0.35) +
      labs(title = paste(location_name), x = "", y = y_axis_labels[[var]]) +
      guides(fill = "none") +
      scale_fill_manual(values = colors) +
      theme_bw() +
    ...
    if (sig_grp_key == "Bonney_ngDNA_per_g_soil") {
      # Manually enter significance groupings
      sig_grp <- data.frame(
        treatment = c("bulk", "CTRL", "N", "P", "NP"),
        sig_grps = c("a", "ab", "b", "c", "d")  # Replace these with your manual assignments
      )
    } else {
      sig_grp <- sig_groups_emmeans_exp[[sig_grp_key]]
    }
    ...
    if (!is.null(sig_grp) && nrow(sig_grp) > 0) {
      label.y <- y_max  # Place significance groupings at the very top of the graph

      p <- p + geom_text(data = sig_grp, aes(x = treatment, y = label.y, label = sig_grps),
                         size = 6, fontface = "bold", color = "black")
    }

    # Store plot in list and print it
    plot_name <- paste(location_name, var, sep = "_")
    treatment_plots_dna[[plot_name]] <- p
    print(p)

    # Save plot in multiple formats with correct names
    file_name <- paste0("MDV-soils-nutrient-limitation-Expr-", var, "_combined")

    ggsave(filename = file.path(sub.directory.1.1.2.2.2.fp, paste0(file_name, ".jpeg")), plot = p, width = 10, height = 6, dpi = 300)
    ggsave(filename = file.path(sub.directory.1.1.2.2.2.fp, paste0(file_name, ".pdf")), plot = p, width = 10, height = 6)
    ggsave(filename = file.path(sub.directory.1.1.2.2.2.fp, paste0(file_name, ".svg")), plot = p, width = 10, height = 6)
  }
}

```

```{r plot dna for combine}
# Initialize list to store plots
treatment_plots_dna_2 <- list()
...
for (var in selected_vars_dna) {
  for (location_name in c("Bonney", "Hoare", "Fryxell")) {
    ...
    # Determine title based on variable and location
    plot_title <- paste(location_name, "Basin")

    # Adjust y-axis settings properly
    if (var %in% c("ngDNA_per_g_soil") & location_name %in% c("Hoare", "Fryxell")) {
      y_axis_title <- element_blank()
      y_axis_text <- element_blank()
    } else {
      y_axis_title <- element_text(size = 16)
    ...
    p <- ggplot(df_var_filtered, aes(x = treatment, y = .data[[var]], fill = treatment)) +
      ...
      guides(fill = "none") +
      scale_fill_manual(values = colors) +
      scale_y_continuous(limits = y_axis_limits_d[[var]], breaks = y_breaks_d[[var]]) +
      theme_bw() +
      theme(
        plot.title = element_text(size = 14),
        axis.title.x = element_blank(),
    ...
    if (!is.null(sig_grp) && nrow(sig_grp) > 0) {
      label.y <- y_axis_limits_d[[var]][2] - (y_axis_limits_d[[var]][2] * 0.01)

      p <- p + geom_text(data = sig_grp, aes(x = treatment, label = sig_grps), y = label.y, 
                         size = 6, fontface = "bold", color = "black")
    }
    ...
    # Use simplified key for storing in list
    location_var <- paste(location_name, var, sep = "_")
    treatment_plots_dna_2[[location_var]] <- p
...
```{r plot combine dna}
#combine plots
combined_dna_all <- treatment_plots_dna_2[["Bonney_ngDNA_per_g_soil"]] + treatment_plots_dna_2[["Hoare_ngDNA_per_g_soil"]] + treatment_plots_dna_2[["Fryxell_ngDNA_per_g_soil"]] + plot_layout(ncol = 3)

# Print the combined plot to the console
print(combined_dna_all)

# Define the file path dynamically
file_path <- file.path(sub.directory.1.1.2.2.fp,
                       paste0("MDV-soils-nutrient-limitation-Expr_DNA_combined_plot"))

# Save the combined plot in multiple formats
ggsave(filename = paste0(file_path, ".jpeg"), plot = combined_dna_all, width = 9, height = 6, dpi = 300)
ggsave(filename = paste0(file_path, ".pdf"), plot = combined_dna_all, width = 9, height = 6)
ggsave(filename = paste0(file_path, ".svg"), plot = combined_dna_all, width = 9, height = 6)
